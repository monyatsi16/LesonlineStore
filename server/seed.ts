...
import ws from "ws";
import { products, priceRecommendations, salesData, users } from "@shared/schema";
...
async function seed() {
  if (!process.env.DATABASE_URL) {
    ...
    console.error("DATABASE_URL=postgresql://postgres:123456@localhost:5432/lesonline");
    process.exit(1);
  }
  ...
  const [demoUser] = await db.insert(users).values({
    email: "demo@smartprice.co.ls",
    password: hashedPassword,
    name: "Demo User",
    businessName: "SmartPrice Demo Store",
  }).returning();
  ...
  await db.insert(salesData).values([
    { userId, month: "Jan", revenue: 45000, orders: 24 },
    { userId, month: "Feb", revenue: 38000, orders: 19 },
    { userId, month: "Mar", revenue: 52000, orders: 31 },
    { userId, month: "Apr", revenue: 47800, orders: 29 },
    { userId, month: "May", revenue: 61000, orders: 38 },
    { userId, month: "Jun", revenue: 55900, orders: 34 },
    { userId, month: "Jul", revenue: 69500, orders: 43 },
  ]);

  console.log("Database seeded successfully!");
  console.log("Demo account: demo@smartprice.co.ls / demo123");
  console.log("Demo products, price recommendations, and sales data have been created.");
